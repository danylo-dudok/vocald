...
try:
    import torch
except ImportError:  # slim image: no torch — disk prompt cache degrades to memory-only
    torch = None
...
def get_cached_voice_prompt(
    ...
    # Check disk cache (torch serialization — unavailable in the slim image)
    if torch is None:
        return None
...
def cache_voice_prompt(
    ...
    if torch is None:
        return  # memory-only in the slim image
