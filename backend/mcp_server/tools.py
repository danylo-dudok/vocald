...
def register_tools(mcp: FastMCP) -> None:
    ...
    @mcp.tool(
        ...
        description=(
            "Create a voice profile. For a preset voice pass "
            "voice_type='preset' with preset_engine (e.g. 'kokoro') and "
            "preset_voice_id (e.g. 'af_bella' — see GET "
            "/profiles/presets/{engine}). For a cloned voice pass "
            "voice_type='cloned' and upload samples afterwards via POST "
            "/profiles/{id}/samples — only clone voices you have consent "
            "to use (see RESPONSIBLE_USE.md)."
        ),
    )
    async def voicebox_create_profile(
        ...
        # Default engine only applies to presets — cloned/designed profiles
        # must leave preset_engine unset or the service rejects them.
        if voice_type == "preset" and preset_engine is None:
        ...
        try:
            ...
            profile = await profiles_service.create_profile(data, db)
            return {
                "id": profile.id,
                "name": profile.name,
    ...
    @mcp.tool(
        name="voicebox.generate",
        description=(
            "Generate speech from text in a voice profile and wait for the "
            "audio artifact. Returns {generation_id, status, duration, "
            "audio_url}; download audio_url (GET /audio/{generation_id}) "
            "with the same Authorization header. Unlike voicebox.speak, "
            "nothing is played on the server — this is the headless path. "
            "The first generation with a new engine downloads its model and "
            "can take minutes; later calls take seconds."
        ),
    )
    async def voicebox_generate(
        ...
        try:
            ...
            if vp is None:
                raise ValueError(
                    "No voice profile resolved. Pass `profile=` with a "
                    "voice profile name or id (see voicebox.list_profiles)."
                )
            started = await _speak(
                profile_id=vp.id,
                profile_name=vp.name,
                text=text,
                engine=engine,
            ...
            while True:
                ...
                status = (gen.status or "completed") if gen else "not_found"
                ...
                if asyncio.get_running_loop().time() > deadline:
                    return {
                        ...
                        "audio_url": None,
                        "error": (
                            f"Still {status} after {timeout_s}s — poll "
                            f"/history/{generation_id} and fetch "
                            f"/audio/{generation_id} once completed."
                        ),
                    }
            ...
            if status != "completed":
                return {
                    ...
                    "status": status,
                    "audio_url": None,
                    "error": getattr(gen, "error", None) or status,
                }
            return {
                "generation_id": generation_id,
                "status": "completed",
                ...
                "profile": vp.name,
                "audio_url": f"/audio/{generation_id}",
            }
        finally:
            db.close()
...
async def _transcribe_file(
    ...
    # load_audio is sync; keep the event loop responsive. Duration is
    # best-effort: compressed formats (mp3/m4a) have no ffmpeg CLI in the
    # slim image, but whisper still decodes them itself via PyAV.
    try:
    ...
    if (
        ...
        raise ValueError(
            f"Whisper model '{model_size}' is not yet downloaded. POST an "
            "audio file to /transcribe once — it downloads the model in the "
            "background — then retry this tool."
        )
