...
class PyTorchQwenLLMBackend:
    ...
    def _load_model_sync(self, model_size: str) -> None:
        try:
            ...
            # The slim headless image ships without torch/transformers —
            # personality rewrite and other LLM features are unavailable.
            raise RuntimeError(
                "LLM features (personality rewrite, compose, refinement) "
                "are not available in the slim headless image: it ships "
                "without torch/transformers. Use personality=false, or "
                "build an image that installs torch + transformers."
            ) from exc
