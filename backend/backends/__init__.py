...
# CTranslate2 (faster-whisper) conversions — same size keys as always.
WHISPER_HF_REPOS = {
...
# Supported TTS engines — kokoro only in the headless slim image.
TTS_ENGINES = {
    "kokoro": "Kokoro",
}
...
def _get_tts_configs() -> list[ModelConfig]:
    """Return TTS model configs (kokoro only in the slim image)."""
    return [
        ModelConfig(
            model_name="kokoro",
...
def _get_whisper_configs() -> list[ModelConfig]:
    """Return Whisper STT model configs (faster-whisper CT2 repos)."""
    return [
        ModelConfig(
            model_name=f"whisper-{size}",
            display_name=f"Whisper {size.capitalize()}",
            engine="whisper",
            hf_repo_id=repo,
            model_size=size,
        )
        for size, repo in WHISPER_HF_REPOS.items()
    ]
...
def get_all_model_configs() -> list[ModelConfig]:
    ...
    return _get_tts_configs() + _get_whisper_configs() + _get_qwen_llm_configs()


def get_tts_model_configs() -> list[ModelConfig]:
    """Return only TTS model configs."""
    return _get_tts_configs()
...
async def ensure_model_cached_or_raise(engine: str, model_size: str = "default") -> None:
    ...
    if not backend._is_model_cached():
        cfg = next((c for c in get_tts_model_configs() if c.engine == engine), None)
...
def get_tts_backend() -> TTSBackend:
    """Get or create the default TTS backend instance (kokoro)."""
    return get_tts_backend_for_engine("kokoro")
...
def get_stt_backend() -> STTBackend:
    ...
    if _stt_backend is None:
        ...
        if backend_type == "mlx":
            ...
            from .fasterwhisper_backend import FasterWhisperSTTBackend

            _stt_backend = FasterWhisperSTTBackend()
